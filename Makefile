...
OBJ =  system/entry.o
OBJ += system/start.o
OBJ += system/syscall_handler.o
OBJ += driver/mmu.o
OBJ += driver/mmu_asm.o
#Lib
OBJ += lib/log.o
OBJ += lib/vprintf.o
OBJ += lib/bit_util.o
...
# Abgabe Dateien
SUBMISSION_FILES = $(shell find . -name '*' -not -name '*.tar.gz' -not -name '.*' -not -type d -not -name '*.pdf' -not -name '*.o' -not -name '*.o_d*' -not -path './.*' -not -name '.DS_Store')
...
qemu_monitor: kernel
	qemu-system-arm -M raspi2 -nographic -monitor telnet::45454,server,nowait -serial mon:stdio -kernel $<

qemu_debug: kernel_debug
	qemu-system-arm -M raspi2 -nographic -s -S -kernel $<

qemu_debug_monitor: kernel_debug
	qemu-system-arm -M raspi2 -nographic -s -S -monitor telnet::45454,server,nowait -serial mon:stdio -kernel $<
